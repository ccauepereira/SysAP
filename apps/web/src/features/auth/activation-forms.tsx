...
export function ActivationOTPForm({ channel }: Readonly<{ channel: "sms" | "email" }>) {
  const router = useRouter(); const [code, setCode] = useState(""); const [error, setError] = useState(""); const [loading, setLoading] = useState(false);
  async function submit(event: FormEvent<HTMLFormElement>) {
    event.preventDefault(); const enrollment = sessionStorage.getItem("sysap-activation-enrollment");
    if (!enrollment || code.length !== 6) { setError("Informe o código de seis dígitos."); return; }
    setLoading(true); setError("");
    try {
      const endpoint = channel === "sms" ? "/api/activation/verify-sms" : "/api/activation/email/verify";
      const response = await fetch(endpoint, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ enrollment_number: enrollment, code }) });
      if (!response.ok) { setError(messageFor(response.status)); return; }
      if (channel === "sms") {
        const emailStart = await fetch("/api/activation/email/start", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ enrollment_number: enrollment }) });
        if (!emailStart.ok && emailStart.status !== 202) { setError(messageFor(emailStart.status)); return; }
        router.push("/ativar/email");
      } else router.push("/ativar/senha");
    } catch { setError("Não foi possível confirmar agora. Tente novamente."); } finally { setLoading(false); }
  }
  return <form className={styles.form} onSubmit={submit}><OTPField disabled={loading} onComplete={setCode} /><p className={styles.timer} aria-live="polite">Código válido por pouco tempo.</p>{error ? <p className={styles.formError} role="alert">{error}</p> : null}<button className={styles.primary} disabled={loading || code.length !== 6} type="submit">{loading ? "Verificando…" : <>Continuar <ArrowRight size={18} /></>}</button></form>;
}
