...
func setupInvitationIntegrationTest(t *testing.T) (*database.Pool, func(), uuid.UUID, uuid.UUID, uuid.UUID) {
	t.Helper()
	...
	queries := []struct {
		...
		{"insert into app.profiles (id, auth_user_id, full_name) values ($1, $2, 'Owner')", []any{profileOwner, uuid.New()}},
		{"insert into app.profiles (id, auth_user_id, full_name) values ($1, $2, 'Trainer')", []any{profileTrainer, uuid.New()}},
	...
	return pool, teardown, orgID, profileOwner, profileTrainer
}

func TestCreateAthleteInvitation(t *testing.T) {
	pool, teardown, orgID, profileOwner, profileTrainer := setupInvitationIntegrationTest(t)
	defer teardown()
